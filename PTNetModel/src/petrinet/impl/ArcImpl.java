...
public class ArcImpl extends MinimalEObjectImpl.Container implements Arc {
	...
	public boolean isEnabled() {
		...
		if (node instanceof Place) {
			if(((Place) node).getTokens().size() < this.getWeight())
				return false;
		} else if ((node = this.getTarget()) instanceof Place) {
			if(!((Place) node).hasCapacity(this.getWeight()))
				return false;
		}
		return true;
	}
	...
	public Object eInvoke(int operationID, EList<?> arguments) throws InvocationTargetException {
		switch (operationID) {
			case PetrinetPackage.ARC___IS_ENABLED:
				return isEnabled();
		}
